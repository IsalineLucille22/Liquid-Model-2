...
Hill_Coeff_1 = 1/(1 + (threshold(1)/z(6))^k_1);
Hill_Coeff_2 = 1/(1 + (threshold(2)/z(5))^k_2);
dx_1 = (2*kappa(1,2) + kappa(1,3))*z(3) - kappa(1,1)*z(1)*z(7) - Hill_Coeff_1*kappa(1,4)*z(6)*z(1);
dx_2 = (2*kappa(2,2) + kappa(2,3))*z(4) - kappa(2,1)*z(2)*z(7) - Hill_Coeff_2*kappa(2,4)*z(5)*z(2);
dy_1 = -(kappa(1,2) + kappa(1,3))*z(3) + kappa(1,1)*z(1)*z(7) + Hill_Coeff_1*kappa(1,4)*z(6)*z(1);
dy_2 = -(kappa(2,2) + kappa(2,3))*z(4) + kappa(2,1)*z(2)*z(7) + Hill_Coeff_2*kappa(2,4)*z(5)*z(2);
dw_1 = kappa(1,3)*z(3) - Hill_Coeff_2*kappa(2,4)*z(5)*z(2);
dw_2 = kappa(2,3)*z(4) -  Hill_Coeff_1*kappa(1,4)*z(6)*z(1);
dr = -(t >= t_1)*kappa(1,1)*z(1)*z(7) - (t >= t_2)*kappa(2,1)*z(2)*z(7);
du_1 = Hill_Coeff_2*kappa(2,4)*z(5)*z(2); %Used by PPU
du_2 = Hill_Coeff_1*kappa(1,4)*z(6)*z(1)*z(6); %Used by PVE
