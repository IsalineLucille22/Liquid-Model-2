function dx = fun_Hill_HandlingTimev2(t, z, kappa, threshold)
...
% z(z<=0) = 0;
Lag_T = (t >= 30);
dx_1 = (2*kappa(1,2) + kappa(1,3) + Lag_T*(2*kappa(3,2) + kappa(3,3)))*z(3) - kappa(1,1)*z(1)*z(7) - Lag_T*kappa(3,1)*z(1)*z(8) - Hill_Coeff_1*kappa(1,4)*z(1)*z(6);
dx_2 = (2*kappa(2,2) + kappa(2,3))*z(4) - kappa(2,1)*z(2)*z(8) - Hill_Coeff_2*kappa(2,4)*z(2)*z(5);
dy_1 = -(kappa(1,2) + kappa(1,3) + Lag_T*(kappa(3,2) + kappa(3,3)))*z(3) + kappa(1,1)*z(1)*z(7) + Lag_T*kappa(3,1)*z(1)*z(8) + Hill_Coeff_1*kappa(1,4)*z(1)*z(6);
dy_2 = -(kappa(2,2) + kappa(2,3))*z(4) + kappa(2,1)*z(2)*z(8) + Hill_Coeff_2*kappa(2,4)*z(2)*z(5);
dw_1 = kappa(1,3)*z(3) + Lag_T*kappa(3,3)*z(3) - Hill_Coeff_2*kappa(2,4)*z(2)*z(5);
...
dr_2 = -kappa(2,1)*z(2)*z(8) - Lag_T*kappa(3,1)*z(1)*z(8);
